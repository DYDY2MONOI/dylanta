<template>
  <div class="animated-background-container">
    <canvas
      ref="canvasRef"
      :class="cn('fixed inset-0 w-full h-full pointer-events-none opacity-95', props.class)"
...
onMounted(() => {
  ...
  setTimeout(() => {
    if (initOGL()) {
      ...
      console.warn("AnimatedBackground: Failed to initialize OGL, background will not be animated");
    }
  }, 100);
});
